 const movies = [
    {
      name: ["Hobbit", "The Hobbit", "Lord of the Rings","LOTR","Le seigneur des anneaux"],
      image: "./img/img24.jpg",
      hint: "In a hole in the ground there lived a ?"
    },
    {
      name: ["Star Wars"],
      image: "./img/img25.jpg",
      hint: "It's a planet with two suns"
    },
    {
      name: ["Friends", "Friend"],
      image: "./img/img12.jpg",
      hint: "Monica and Chandler spent time here"
    },
    {
        name: ["Breaking Bad", "Breakings Bad"],
        image: "./img/img2.jpg",
        hint: "He was tracking drug dealers and collecting minerals in this house"
      },
      {
        name: ["Parasite", "Parasites"],
        image: "./img/img5.jpg",
        hint: "There is a super creepy basement in this house"
      },
      {
        name: ["The Walking Dead", "Walking Dead"],
        image: "./img/img19.jpg",
        hint: "The main protagonist wakes up from his coma in this hospital"

      },
      {
        name: ["The Office", "Office"],
        image: "./img/img17.jpg",
        hint : "Behind this door, they sell paper"
      },
      {
        name: ["Shining", "The Shining", "Shinings"],
        image: "./img/img9.jpg",
        hint :"Just an haunted hotel, THE place for a writer in need of inspiration"
      },
      
      {
        name: ["American Horror Story", "AHS"],
        image: "./img/img23.jpg",
        hint: "One season takes place in an asylum, An anthology series, initials are enough"
      },
      {
        name: ["Alien", "Aliens"],
        image: "./img/img15.jpg",
        hint : "In space no one can hear you scream" 
      },
      {
        name: ["Desperate Housewives", "Desesperate Housewives", "Desperate Housewive", "Desperates Housewive", "Desperates Housewives"],
        image: "./img/img8.jpg",
        hint : "Really, you haven't recognized Bree Van De Kamp's house?"
      },
      {
        name: ["The Haunting of Hill House", "The Haunting Of", "Haunting Of", "Haunting Of Hill House", "Hill Bouse", "Bly manor", "The Haunting of Bly Manor"],
        image: "./img/img22.jpg",
        hint: "This house is so haunted that it's the name of the show"
      },
      {
        name: ["Stranger Things", "Stranger Thing"],
        image: "./img/img16.jpg",
        hint: "Before Vecna"
      },
      {
        name: ["The Boys", "The Boy"],
        image: "./img/img20.png",
        hint: "The Homelander loves this room."
      },
      {
        name: ["Dexter"],
        image: "./img/img1.jpg",
        hint: "Just a Miami's serial killer (who kills serial killer) home"
      },
      
      {
        name: ["Shameless"],
        image: "./img/img21.jpg",
        hint: "An 11-season series about kids with an alcoholic father"
      },
      {
        name: ["Parks and Recreation", "Parks and Rec", "Park and Rec", "Park and Recreation", "Park and Recreations"],
        image: "./img/img14.jpg",
        hint: "A sitcom with Chris Pratt"
      },
      {
        name: ["Mad Men", "Mad Mens"],
        image: "./img/img4.jpg",
        hint: "It's about ads and dramas"
      },
      {
        name: ["Better Call Saul", "Call Saul", "Better Call"],
        image: "./img/img18.jpg",
        hint: "A netflix spin-off, It's all good man"
      },
      {
        name: ["Tenet"],
        image: "./img/img6.jpg",
        hint: "There was a chase with inverted cars on this highway"
      },
      {
        name: ["The Big Lebowski", "Big Lebowski", "Big Lebowsky", "The Big Lebowsky", "Lebowski", "Lebowsky"],
        image: "./img/img7.jpg",
        hint: "I think they are spreading ashes here"
      },
      
      {
        name: ["American Psycho"],
        image: "./img/img10.jpg",
        hint: "It really looks like the appartment of a psycho"
      },
      {
        name: ["The Sopranos", "Sopranos", "The Soprano"],
        image: "./img/img3.png",
        hint: "It's about a gangster who sees a shrink, oh and his name is Tony"
      },
      {
        name: ["Monk"],
        image: "./img/img13.jpg",
        hint: "A private detective with compulsive habits and phobias"
      },
      {
        name: ["Pulp Fiction"],
        image: "./img/img3.jpg",
        hint: "Butch has to get his gold watch"
      },
      {
        name: ["Batman Begins", "The Dark Knight", "Dark Knight", "Batman"],
        image: "./img/img11.jpg",
        hint: "The mansion of an extremely rich man with a double identity"
      },
     
  ];